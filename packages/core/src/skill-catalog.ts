 ...
 * how to invoke, what they produce). The actual Hermes plugin tool
 * implementations are stubs in `packages/hermes-plugin-holon-owner/`
...
export interface SkillDescriptor {
  /** Stable kebab-case id — also the Hermes plugin tool name. */
  id: string;
  ...
  consults?: string[];
  /** True if the underlying Hermes plugin tool exists and is wired.
...
export const SKILL_CATALOG: SkillDescriptor[] = [
  ...
  },
  {
    ...
    // Wired via gmail_summarize_inbox in packages/hermes-plugin-holon-owner/
    // tools.py — composes gmail_list_threads + gmail_read_thread + DeepSeek.
   ...
   *
   * Wiring posture (V1):
   *   `consults`: descriptor array — reference ids the LLM may retrieve.
   *   PULL path (LLM-invoked consult_reference tool): SHIPPED at
   *     331d10e / TD-014 RESOLVED. The Hermes plugin registers
   *     consult_reference(reference_id) → GET /api/v1/references/<id>
   *     → returns markdown summary as tool result. LLM invokes this
   *     tool itself when the user asks a META question.
   *   PUSH path (auto-inject consults summary into pre_llm_call hook):
   *     NOT implemented — the pre_llm_call hook today only injects the
   *     workspace snapshot (inject_workspace_snapshot). Auto-quoting
   *     consults references without an LLM tool-call round-trip is
  ...
  /* ── Adopted from Hermes bundled tools ──────────────────────────────
   * 2026-05-17: user asked to pull in Hermes's built-in capabilities.
   * Each lands here as a UI scaffold; the wiring to the actual Hermes
...

  {
    ...
    description:
      'Generates images from a prompt. Pick aspect ratio, style, and quantity. Returns a .png file (or several). Wires to Hermes\'s image_generation_tool — provider configured per the worker dispatcher\'s env. Use for slide illustrations, social posts, mockups, or "show me what X could look like" sketches.',
    examples: [
  ...
  },
  {
    ...
    description:
      'Generates short video clips (typically 5-15s) from a text prompt or seed image. Slow + token-expensive — use sparingly. Returns a .mp4 file. Wires to Hermes\'s video generation tool stack.',
    examples: [
      '做一个 5 秒的产品演示动效',
  ...
  },
  {
    ...
    description:
      'Opens a URL in a headless Chromium, returns rendered text + DOM-extracted structured data, and can take screenshots. Use for: reading articles behind JS-heavy pages, scraping a single page, capturing a screenshot for a report. Backed by Hermes\'s browser_tool / browser_cdp_tool.',
    examples: [
      '把这个网页的主要内容提取出来',
  ...
  },
  {
    ...
    description:
      'Executes Python (and small shell snippets) in a sandbox. Use for quick calculations, file transformations, one-off scripts, or testing snippets before pasting them somewhere. Backed by Hermes\'s code_execution_tool. The Desk AI picks this when a question is faster to compute than to derive.',
    examples: [
  ...
  },
  {
    ...
    description:
      'Reads and writes Feishu documents, spreadsheets, and Drive files. Use for: pulling meeting notes, syncing reports into a shared workspace, or posting deliverables to the team\'s Feishu workspace. Wires to Hermes\'s feishu_doc_tool / feishu_drive_tool — needs auth.',
    examples: [
      '把今天的工作日志同步到 Feishu',
  ...
  },
  {
    ...
    description:
      'Joins a Google Meet (as the owner or observer), captures the transcript, then produces a structured summary + action items. Use for: catching up on a call you missed, or having a written record of one you attended. Backed by Hermes\'s google_meet plugin — needs OAuth.',
    examples: [
      '帮我录这个会议并生成纪要',
  ...
  },
  {
    ...
    description:
      'Creates cards, moves them across columns, and queries the board state. Use as a lightweight task tracker — the owner\'s personal next-actions board, separate from the Holon mission inbox (which is cross-desk handoffs). Backed by Hermes\'s kanban_tools.',
    examples: [
  ...
  /* ── Meta-skill: task decomposition (plan-and-execute) ──────────── */
  {
    ...
    examples: [
      ...
      'Plan the migration from DeepSeek to Claude 4 across our worker dispatcher',
    ],
  ...
  /* ── Meta-skills: agent CRUD ────────────────────────────────────── */
  {
    ...
    tags: ['team', 'hire', 'meta'],
    description:
      'Take a sketched role description from the owner and produce a complete new staff record — name, role_label, system_prompt, max_concurrent_jobs, denied_skills (deny-list against the CEO catalog), monthly_budget_millicents, proxy_staff_id. Persists via the existing create_staff Hermes tool. Use this when the owner says "hire me a market researcher" or "make a teammate who handles refunds".',
    examples: [
  ...
  /* ── Meta-skills: template CRUD (mirrors skill CRUD) ───────────── */
  {
    ...
    description:
      'Create a new template (output-format reference). Two flows: (a) describe what kind of fillable shell you want and the LLM drafts the body + variables; (b) paste an existing markdown body + tag the {{placeholder}} variables. Lands in the user-defined templates store; surfaced on /references under kind=output-format.',
    examples: [
  ...
  },
  {
    ...
    description:
      'Create a new reference entry. Two flows: (a) describe the standard / spec / regulation and the LLM drafts the descriptor (name, authority, version, summary, key sections) — owner provides the URL or asks LLM to search; (b) paste the URL + a one-line tagline and the LLM expands. Lands in the user-defined references store; surfaced on /references under the matching kind.',
    examples: [
  ...
  },
  {
    ...
    description:
      'Posts a message to a Discord channel, reads recent channel history, or pings a user. Use for: cross-posting deliverables to a community, dropping status updates into a working channel, or catching up on missed conversation. Backed by Hermes\'s discord_tool — needs a bot token.',
    examples: [
 ...
 *     to implemented:false (the Hermes wiring lands later).
...
export function createSkill(input: CreateSkillInput): SkillDescriptor {
  ...
  const record: SkillDescriptor = {
    ...
    ...(Array.isArray(input.consults) && input.consults.length > 0
      ...
      : {}),
    // New skills default to scaffold — the Hermes plugin wiring hasn't
