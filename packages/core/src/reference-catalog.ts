...
export const REFERENCE_CATALOG: ReferenceDescriptor[] = [
   ...
   * source_type='inline'; logged in TECH-DEBT.md. */
  {
    ...
    summary:
      ...
      '- **Staff** — an employee on your desk. Three substrates: AI (LLM-backed, runs in Hermes), human (real person reachable via email/chat), CLI (local terminal agent for code/ops). Flat roster — staff never own other staff.\n' +
      ...
      '\n' +
      '## Where to go next\n' +
      '- **/me** — your owner profile, language, LLM provider config, integrations (Gmail, Feishu, …).\n' +
      '- **/members** — your team.\n' +
  ...
  },
  {
    id: 'ref-holon-faq',
    name: 'Holon FAQ',
    tagline: 'First-week common questions — hire, language, LLM, Gmail, slow chat',
    icon: '❓',
    kind: 'company-internal',
    ...
    summary:
      ...
      'Go to **/members** → click **+ Hire** (top-right). Fill in name, role label, system prompt, max concurrent jobs, monthly budget. AI staff need an LLM provider configured (see /me → LLM providers). Or just ask the Desk AI: "hire me a market researcher with a 100 元 monthly budget" — it invokes the `create_agent` skill.\n' +
      '\n' +
      ...
      'Type `@` in the chat input → typeahead pops up listing your roster → pick one → the message dispatches to that staff\'s private Hermes session instead of the Desk AI. Useful when you want to talk to a specific employee directly (e.g. "@Aria what\'s your draft look like?").\n' +
      '\n' +
      '## Language & profile\n' +
      '\n' +
      ...
      '**/me** → Identity section. Updates the workspace snapshot the pre_llm_call hook injects, so the Desk AI sees the new identity on its next turn.\n' +
      '\n' +
      '## LLM provider\n' +
      '\n' +
      '### How do I use my own OpenAI key instead of Holon\'s default DeepSeek?\n' +
      '**/me** → LLM providers section → find the OpenAI card → click **Add key** → paste, click **Test**, click **Set Active**. The next chat turn routes through OpenAI. Also surfaced inline during onboarding Step 6 — pick "Bring my own API key".\n' +
      '\n' +
      '### Which providers are supported?\n' +
      'Today: OpenAI, Anthropic, DeepSeek, Mistral, Together, Groq, Fireworks, Cohere, OpenRouter, Cerebras, plus Holon-trial (DeepSeek-on-our-key, capped). 11 cards on /me; one Active at a time.\n' +
      '\n' +
      '## Integrations\n' +
      '\n' +
      ...
      'Two layers: (1) Hermes ACP bridge cold-start ~1-2s on first message after dev restart; (2) the LLM provider itself — DeepSeek is typically <500ms time-to-first-token, OpenAI gpt-4o ~1s. Warm subsequent turns: <500ms TTFT. If it\'s >5s consistently, check **/me → LLM providers** that your Active key tested green.\n' +
      '\n' +
      ...
      '**/skills**, **/templates**, **/references** all have a **+ New** button. Two modes per modal: **Direct** (fill the fields yourself) or **Describe** (write a one-paragraph plain-language description; LLM expands into the descriptor). User-defined entries appear under "yours"; built-ins are collapsed under "Examples" at the bottom.\n' +
      '\n' +
  ...
  },
  {
    ...
    summary:
      ...
      '- The message dispatches to **that staff\'s private Hermes session** rather than the Desk AI. Their session is isolated — has its own history, granted_skills, system_prompt.\n' +
      ...
      '- Chat with `@SomeStaff` → routes to **that staff\'s** Hermes session (their persona; their granted_skills; their tool scope).\n' +
