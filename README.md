...
|---|---|---|---|---|---|
| **Primary purpose** | **Boss / manager view of workers** — manage a virtual team with stable members, secretary as buffer, supports micromanagement (attach to any worker's tmux, watch, intervene); don't reinvent the wheel (CLIs do the work) | The actual model loop + tool calling | Minimal markdown-only memory for one agent | Multi-tenant WeChat gateway + routing to LLMs | Personal knowledge base (notes) |
